		...
		<table class="table table-condensed table__prices">
            ...
            {foreach $price_lists_items as $price_list_item}
            	<tr>
                    ...
                    <td><a href="{site_url($price_list_item.price_list_xls)}" target="_blank" class="prices_link prices_link_xls">Скачать xls</a></td>
                    {if $price_list_item.price_list_other}
                    {$other = mb_substr($price_list_item.price_list_other, -3)}
                    	<td><a href="{site_url($price_list_item.price_list_other)}" target="_blank" download="" class="prices_link prices_link_{$other}">Скачать {$other}</a></td>
                    {/if}
